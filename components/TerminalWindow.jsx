...
const TerminalWindow = ({ title, children, className = "", bodyClassName = "" }) => {
  ...
  const handleMouseMove = (e) => {
    ...
    setPos({
      ...
      opacity: 1,
      rotateX: ((e.clientY - rect.top) / rect.height - 0.5) * -2.5,
      rotateY: ((e.clientX - rect.left) / rect.width - 0.5) * 2.5,
    });
  };
  ...
  return (
    <div
      ...
      className={`glass-card relative overflow-hidden transition-transform duration-300 ease-out ${className}`}
      style={{
        transform: pos.opacity ? `perspective(1200px) rotateX(${pos.rotateX}deg) rotateY(${pos.rotateY}deg) translateZ(8px)` : "perspective(1200px) rotateX(0deg) rotateY(0deg)",
